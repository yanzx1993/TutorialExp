...
def gcn_layer(snode_network_feature,input_dim, output_dim, name, act=tf.nn.relu):
    with tf.name_scope(name):
        ...
        with tf.name_scope("biases"):
            biases = bias_init([node, output_dim])
        with tf.name_scope("gcn"):
            ...
            for j in range(output_dim):
                ...
                temp_y=tf.add_n(y)
                x.append(temp_y)
            temp_x=tf.reshape(tf.stack(x, axis=2),[node,output_dim])+biases
        with tf.name_scope("activations"):
            activations=act(temp_x,name="activations")
    return activations


def softmax_layer(input, vnode_features, input_dim, output_dim, name, act=tf.nn.softmax):
    with tf.name_scope(name):
        with tf.name_scope("fc_weights"):
            fc_weights=weight_init([len(vnode_features),output_dim])
        with tf.name_scope("pooling_weights"):
            pooling_weights=weight_init([input_dim,1])
        with tf.name_scope("biases"):
            biases=bias_init([output_dim,1])
        with tf.name_scope("multiply"):
            #pooled_y=pooling_weights+[1,2]
            pooled_y=tf.matmul(input,pooling_weights)+biases
            pooled_y=tf.transpose(pooled_y)
            for i in range(len(vnode_features)):
                pooled_y+=fc_weights[i]*vnode_features[i]
        return act(pooled_y)
...
test=gcn_layer(x,input_features,output_features,"test")
out=softmax_layer(test,[1,2], output_features,node,"out",act=tf.identity)
print("hello")
elapsed = (time.clock() - start)
print("Time used:",elapsed)
elapsed=time.clock()
